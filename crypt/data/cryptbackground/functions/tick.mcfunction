...
execute at @a[nbt={SelectedItem:{id: "minecraft:fishing_rod", Count: 1b, tag: {HideFlags: 2, AttributeModifiers: [{Amount: 1d, Slot: "mainhand", AttributeName: "generic.attack_speed", Operation: 0, UUID: [I; 69, 69, 69, 69], Name: "generic.attack_speed"}], display: {Lore: ['{"text":" "}', '{"text":"when held all rods in the area "}', '{"text":"become attack with gold"}'], Name: '{"text":"Midas rod","color":"gold","bold":true}'}}}}] run execute as @e[type=minecraft:fishing_bobber,distance=1..3] run tag @s add midasbob
execute at @e[type=fishing_bobber,tag=midasbob] run summon falling_block ~ ~ ~ {BlockState:{Name:"minecraft:gold_block"},Time:0,DropItem:0b}
execute at @e[type=fishing_bobber,tag=midasbob] run summon falling_block ~ ~1 ~ {BlockState:{Name:"minecraft:gold_block"},Time:0,DropItem:0b}
execute at @e[type=fishing_bobber,tag=midasbob] run particle minecraft:crit ~ ~2 ~ 0 0 0 2 20 normal
execute at @e[type=fishing_bobber,tag=midasbob] run summon creeper ~ ~ ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,CustomNameVisible:0b,PersistenceRequired:1b,NoAI:1b,powered:0b,ExplosionRadius:1b,Fuse:1,ignited:1b,CustomName:'{"text":"Midas rod","color":"gold","bold":true}',HandItems:[{id:"minecraft:fishing_rod",Count:1b,tag:{display:{Name:'{"text":"Midas rod","color":"gold","bold":true}',Lore:['{"text":" "}','{"text":"when held all rods in the area "}','{"text":"become attack with gold"}']},HideFlags:2,AttributeModifiers:[{AttributeName:"generic.attack_speed",Name:"generic.attack_speed",Amount:1,Operation:0,UUID:[I;69,69,69,69],Slot:"mainhand"}]}},{}]}
...
execute at @e[type=arrow,nbt={CustomPotionEffects:[{Id:15b,Amplifier:1b,Duration:10,ShowParticles:1b}]}] run summon snowball ~ ~ ~ {NoGravity:1b,Tags:["trail"],Item:{id:"minecraft:ghast_tear",Count:1b}}
execute at @e[type=arrow,nbt={CustomPotionEffects:[{Id:16b,Amplifier:1b,Duration:10,ShowParticles:1b}]}] run summon snowball ~ ~ ~ {NoGravity:1b,Tags:["trail"],Item:{id:"minecraft:tnt",Count:1b}}
...
#diamond staff
execute as @a[nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",Count:1b,tag:{CustomModelData:diamond_staff}}},scores={rightclick=1..},tag=!cooldown] at @s run function cryptbackground:diamond_staff
...
execute as @a[nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",Count:1b,tag:{CustomModelData:diamond_staff}}},scores={cooldown=0..60}] run function cryptbackground:diamond_staff_cooldown
...
execute as @a[nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",Count:1b,tag:{CustomModelData:665566}}},scores={cooldown=0..60}] run function cryptbackground:diamond_staff_cooldown
execute as @a[scores={cooldown=61}] run tag @s remove cooldown
...
execute as @a[nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",Count:1b,tag:{CustomModelData:34455}}}] run function end:dooropenkey
...
execute if block 1 69 2 minecraft:end_portal_frame[eye=true] if block -1 69 2 minecraft:end_portal_frame[eye=true] if block -2 69 1 minecraft:end_portal_frame[eye=true] if block -2 69 -1 minecraft:end_portal_frame[eye=true] if block -1 69 -2 minecraft:end_portal_frame[eye=true] if block 1 69 -2 minecraft:end_portal_frame[eye=true] if block 2 69 -1 minecraft:end_portal_frame[eye=true] if block 2 69 1 minecraft:end_portal_frame[eye=true] run execute as @a[x=-1,y=67,z=-1,dx=2,dy=1.6,dz=2] in end:crypt run tp 10 166 0
#gui
